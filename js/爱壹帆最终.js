var rule = {
    ...
    推荐: `js:
        ...
        function yfspKey(v) {
            v = String(v || '');
            var p = v.indexOf('/play/');
            if (p >= 0) v = v.substring(p + 6);
            p = v.indexOf('/watch/');
            if (p >= 0) v = v.substring(p + 7);
            v = v.split('?')[0].split('#')[0];
            if (v.indexOf('/') >= 0 || v.indexOf('http') === 0 || v.length < 6) return '';
            return v;
        }
        function yfspVod(it) {
            var lp = it.listPath || '';
            var id = it.key || it.secretVID || yfspKey(lp);
            return {
                vod_id: id,
        ...
        var json = yfspInject(request('https://www.yfsp.tv/list/drama?region=%E5%A4%A7%E9%99%86', {headers: rule.headers}));
        var list = json && json.playList && json.playList[0] && json.playList[0].listData ? json.playList[0].listData : [];
        var out = [], seen = {};
    ...
    二级: `js:
        ...
        function yfspKey(v) {
            ...
            if (v.indexOf('http') === 0) {
                p = v.indexOf('/');
                if (p >= 0) v = v.substring(p + 1);
            }
            if (v.indexOf('play/') === 0) v = v.substring(5);
            if (v.indexOf('watch/') === 0) v = v.substring(6);
            v = v.split('?')[0].split('#')[0];
            if (v.indexOf('/') >= 0 || v.length < 6) return String(input);
            return v;
        }
        var key = yfspKey(input);
    ...
    搜索: `js:
        ...
        function yfspVod(it) {
            var lp = it.listPath || '';
            var id = it.key || it.secretVID || yfspKey(lp);
            return {vod_id: id, vod_name: it.title || '', vod_pic: it.image || it.imgPath || '', vod_remarks: it.lastName || it.atypeName || ''};
        }
    ...
    lazy: `js:
        ...
        var h = {'User-Agent': rule.headers['User-Agent'], 'Referer': 'https://www.yfsp.tv/', 'Origin': 'https://www.yfsp.tv'};
