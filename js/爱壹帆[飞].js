var rule = {
    ...
    lazy: `js:
        ...
        var data = JSON.parse(request(yfspSign('https://m10.yfsp.tv/v3/video/play?' + qs), {headers: rule.headers}));
        var info = data && data.data && data.data.info && data.data.info[0] ? data.data.info[0] : {};
        var arr = info.flvPathList || [];
        var url = '';
        for (var i = 0; i < arr.length; i++) {
            if (arr[i] && arr[i].isHls && arr[i].result) { url = arr[i].result; break; }
        }
        if (!url && arr.length) url = arr[0].result || arr[0].dashResult || arr[0].rtmp || '';
        if (/\\.m3u8|\\.mp4/i.test(url)) {
            input = {parse: 0, jx: 0, url: url, header: JSON.stringify({'User-Agent': 'Mozilla/5.0', 'Referer': 'https://www.yfsp.tv/'})};
        } else {
            input = {parse: 1, jx: 0, url: 'https://www.yfsp.tv/watch?v=' + encodeURIComponent(ep), header: JSON.stringify({'User-Agent': 'Mozilla/5.0', 'Referer': 'https://www.yfsp.tv/'})};
        }
    `
};
