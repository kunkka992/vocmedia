var rule = {
    ...
    推荐: `js:
        ...
        function yfspVod(it) {
            var lp = it.listPath || '';
            var id = it.key || it.secretVID || ((lp.match(/\/play\/([^/?#]+)/) || [])[1]) || ((lp.match(/\/watch\/([^/?#]+)/) || [])[1]) || '';
            if (!id && /^[A-Za-z0-9_-]{6,}$/.test(lp)) id = lp;
            return {
                vod_id: id,
    ...
    二级: `js:
        ...
        var key = (String(input).match(/\\/play\\/([^/?#]+)/) || [])[1] || (String(input).match(/\\/watch\\/([^/?#]+)/) || [])[1] || String(input).replace(/^https?:\\/\\/[^/]+\\//, '').replace(/^play\\//, '').replace(/^watch\\//, '').split(/[?#]/)[0];
