var rule = {
    ...
    detailUrl: '/api/mw-movie/anonymous/video/detail',
    searchable: 1,
    quickSearch: 1,
    filterable: 1,
    timeout: 30000,
    ...
    `,
    二级: `js:
        var json = jpApi('/mw-movie/anonymous/video/detail', {id: input});
        var it = json.data || {};
        var eps = it.episodeList || [];
        var list1080 = [];
        var list720 = [];
        eps.forEach(function(ep) {
            var name = ep.name || ep.sort || '';
            var nid = ep.nid || ep.id || '';
            if (!nid) return;
            list1080.push(name + '$' + it.vodId + '@' + nid + '@1080');
            list720.push(name + '$' + it.vodId + '@' + nid + '@720');
        });
        VOD = {
            vod_id: it.vodId || input,
            vod_name: it.vodName || '',
            vod_pic: it.vodPic || '',
